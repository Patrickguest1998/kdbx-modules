...
setshardcallbacks:{[resfn;errfn]
  / call after mounting so async postback replies from asyncdispatch resolve correctly
  .z.m.shardresultcallback:resfn;
  .z.m.sharderrorcallback:errfn;
};

/ active request tracking - one row per in-flight client query
requests:([requestid:`u#`long$()]
  time:`timestamp$();
  clienth:`int$();
  remaining:`long$();
  joinfn:();
...
finishrequest:{[reqid;err]
  / stamp completion and drop the shard result accumulator; keeps requests for audit
  .z.m.shardresults:(reqid,())_shardresults;
  update error:err,returntime:cp[] from .z.M.requests where requestid=reqid;
};
...
execquery:{[query;starttime;endtime;joinfn;postback;timeout;sync]
  ...
  shards:update shardquery:buildshardquery[query;;]'[starttime;endtime] from shards;
  reqid:requestid;
...
removerequests:{[age]
  ...
  .z.m.requests:0!delete from .z.m.requests where not null returntime, cp[]>returntime+age;
};
